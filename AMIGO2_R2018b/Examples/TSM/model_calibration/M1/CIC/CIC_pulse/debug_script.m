    ...
    short_name     = strcat('M1_CIC_Pulse',int2str(epcc_exps));
    ...
    y0 = 0.1.*M1_compute_steady_state(global_theta_guess,[50.61466259329, 158.608748834354],[0 25]); %[ToggleSwitch_M1_Compute_SteadyState(model.par,[1000,400],1,0) 0];
    ...
    numPulses = 30;       % Number of pulses    
    inducer = [rand(1,2); randi([0 100],1,2)];  % Extract 2 random integer values from the Uniform distribution in (0,1) ng/mL for IPTG and (0,100) for aTc to be used as low and high values in each cycle
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    newExps.u{1}=repmat(inducer,1,numPulses);              % IPTG values for the input              
    newExps.t_con{1}=union(0:100:duration,20:100:duration);      % Switching times 
