...
results_folder = strcat('M5Fit',datestr(now,'yyyy-mm-dd-HHMMSS'));
short_name     = strcat('M5F',int2str(epcc_exps));
...
for iexp=1:length(exps_indexTraining)
    ...
    Y0(iexp,:) = M5_Compute_SteadyState_OverNight(epcc_exps,inputs,exp_indexData,global_theta_guess,Data.exp_data{1,exp_indexData}(:,1)',Data.Initial_IPTG{1,exp_indexData},Data.Initial_aTc{1,exp_indexData});
end
...
for iexp=1:length(exps_indexTest)
    ...
    Y0_test(iexp,:) = [M5_Compute_SteadyState_OverNight(epcc_exps,inputs,exp_indexData,best_global_theta,Data.exp_data{1,exp_indexData}(:,1)',Data.Initial_IPTG{1,exp_indexData},Data.Initial_aTc{1,exp_indexData}) 0];
end
