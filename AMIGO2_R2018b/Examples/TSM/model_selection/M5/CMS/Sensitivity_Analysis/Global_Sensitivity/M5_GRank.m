...
%============================
inputs.pathd.results_folder='M5_T'; % Folder to keep results (in Results) for a given problem                       
inputs.pathd.short_name='M5_GRank';                       % To identify figures and reports for a given problem 
inputs.pathd.runident='full-par-bounds';                         % [] Identifier required in order not to overwrite previous results
...
% MODEL RELATED DATA
%============================
[model] = ToggleSwitch_load_model_M5();
...
for iexp=1:exps.n_exp
    ...
    Y0(iexp,:) = M5_Compute_SteadyState_OverNight(epcc_exps,inputs,iexp,model.par,Data.exp_data{1,iexp}(:,1)',Data.init_u{iexp});
end
  ...
  for iexp=1:exps.n_exp
    ...
    %exps.exp_y0{iexp}([ 4 5]) = Data.exp_data{1,iexp}(:,1)';  
...
global_theta_min=[0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,0.001,2,1,0.001,0.001,2,1,2,0.01,0.001,0.001,0.001];
global_theta_max=[1,1,100,100,10,100,100,100,100,100,2000,4,1000,200,100,4,100,4,100,100,100,100];

% Excluding parameters which are fixed
param_including_vector=[false,false,false,false,false,false,false,false,false,false,true,true,true,true,true,true,true,true];

% GLOBAL UNKNOWNS (SAME VALUE FOR ALL EXPERIMENTS)
% settings for PE

inputs.PEsol.id_global_theta=model.par_names(param_including_vector,:);
inputs.PEsol.global_theta_min=global_theta_min(param_including_vector);%(1/2).*inputs.model.par(param_including_vector); 
inputs.PEsol.global_theta_max=global_theta_max(param_including_vector); %2.*inputs.model.par(param_including_vector); 
