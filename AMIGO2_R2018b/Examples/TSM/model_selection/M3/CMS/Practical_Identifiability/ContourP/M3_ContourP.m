...
%============================
inputs.pathd.results_folder='M3_ToggleSwitch'; % Folder to keep results (in Results) for a given problem                       
inputs.pathd.short_name='M3_ContourP';                       % To identify figures and reports for a given problem 
...
for iexp=1:exps.n_exp
    ...
    Y0(iexp,:) = M3_Compute_SteadyState_OverNight(epcc_exps,inputs,iexp,model.par,Data.exp_data{1,iexp}(:,1)',Data.init_u{iexp});
end
  ...
  for iexp=1:exps.n_exp
    ...
    exps.n_obs{iexp} = 2; 
    exps.obs_names{iexp} = char('LacI_M3','TetR_M3');
    exps.obs{iexp} = char('LacI_M3 = LacI','TetR_M3 = TetR');
...
%global_theta_min=[0.1386,5e-06,1e-3,7000,1,2,2,0.0165,1e-05,0.001,700,3.07e-4,2,2,1.32,1,0.001,0.001,1.32,1]; % verify Theta_T is correct 
%global_theta_max=[0.1386,10,1000,7000,100,4,4,0.0165,30,1000,700,3.07e-2,4,4,1320,1000,0.1,0.1,1320,1000]; % Maximum allowed values for the parameters
...
inputs.PEsol.global_theta_max=2.*inputs.model.par(param_including_vector); %global_theta_max(param_including_vector);  % Maximum allowed values for the parameters
inputs.PEsol.global_theta_min=0.5.*inputs.model.par(param_including_vector); %global_theta_min(param_including_vector);  % Minimum allowed values for the parameters
